...
# general info
#registrant
netName=$(grep -i 'netName:' IPresults.txt|  head -n1 | cut -d: -f2-)
...
if [ $dServer ]; 
then
    regName=$(grep 'Registrant Name:' DNSresult.txt|  head -n1 | cut -d: -f2-)
...
wHostCity=`whois $wHostIP -H| grep -i 'City:'|  head -n1  | cut -d: -f2-`
...
wHostAbuseEmail=`whois $wHostIP -H| grep 'OrgAbuseEmail:'|  head -n1  | cut -d: -f2-`
wHostAbusePhone=`whois $wHostIP -H| grep 'OrgAbusePhone:'|  head -n1  | cut -d: -f2-`
